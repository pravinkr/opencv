...
def get3DImageFromDisparity(imgL, imgR):
    ...
    # disp = (left_matcher.compute(imgL, imgR)/16.0).astype(np.float32)
   ...
   # print('generating 3d point cloud...',)
    ...
    print("disparity map astype: ",type(disp))
...
if __name__ == '__main__':
    ...
    # python point_cloud.py --calibration_file="Calibration_MatrixStereo.yaml" --left_source="./left_camera/imageL_0.png" --right_source="./right_camera/imageR_0.png" --is_real_time=True
    # python point_cloud.py --calibration_file="Calibration_MatrixStereo.yaml" --left_source="/dev/video2" --right_source="/dev/video4" --is_real_time=True
    ...
    while True:  # Loop until 'q' pressed or stream ends
        ...
        # time.sleep(5) # get 1 frame every 5 seconds
        ...
        print(f'Size: {height} x {width} x {channel}')
        ...
        # get3DImageFromDisparity(gray_left,gray_right)
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'):  # Get key to stop stream. Press q for exit
            ...
            print("disparity map astype: ",type(disparity_image))
            points3D = cv2.reprojectImageTo3D(disparity_image, Q, handleMissingValues=False)
