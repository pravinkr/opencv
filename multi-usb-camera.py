...
camera_left = cv2.VideoCapture("/dev/video2") ### Left USB Camera
camera_right = cv2.VideoCapture("/dev/video4") ### Right USB Camera

camera_left.set(3,640)             # set frame width
camera_left.set(4,370)              # set frame height

camera_right.set(3,640)             # set frame width
camera_right.set(4,370)              # set frame height


time.sleep(0.5)

print(camera_left.get(3),camera_left.get(4))
print(camera_left.get(5))

print(camera_left.get(3),camera_left.get(4))
print(camera_left.get(5))

print(camera_right.get(3),camera_right.get(4))
print(camera_right.get(5))
...
while 1:
    ...
    # grab a frame
    (grabbed_left,frame0_left) = camera_left.read()
    
    # end of feed
    if not grabbed_left:
        print("No left frame detected")
        break

    # grab a frame
    (grabbed_right,frame0_right) = camera_right.read()
    
    # end of feed
    if not grabbed_right:
        print("No right frame detected")
        break
    ...
    # display
    cv2.imshow("Frame0_left: Raw",frame0_left)
    cv2.imshow("Frame0_right: Raw",frame0_right)
